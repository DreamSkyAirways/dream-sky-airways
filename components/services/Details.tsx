          ...
          return (
            <div
              ...
              className={`w-full bg-white border-2 rounded-3xl overflow-hidden transition-all duration-300 shadow-sm ${
                isExpanded ? "border-red-600 shadow-red-100/50" : "border-gray-200 hover:border-red-500"
              }`}
            >
              {/* COLLAPSED STATE */}
              <div
                onClick={() => toggleDropdown(srv.id)}
                className="min-h-[20vh] p-6 sm:p-8 flex flex-col justify-between cursor-pointer bg-white hover:bg-neutral-50/70 transition"
              >
                {/* Top Bar */}
                <div className="flex items-center justify-between w-full">
                  <div className="flex items-center gap-3">
                    <span
                      className={`text-2xl sm:text-3xl font-extrabold font-mono transition-colors ${
                        isExpanded ? "text-red-600" : "text-black"
                      }`}
                    >
                      {srv.code}
                    </span>
                    <span
                      className={`px-3.5 py-1 rounded-full text-xs font-bold uppercase tracking-wider transition-colors ${
                        isExpanded ? "bg-red-600 text-white" : "bg-black text-white"
                      }`}
                    >
                      {srv.badge}
                    </span>
                  ...
                  <div
                    className={`w-10 h-10 rounded-full border-2 flex items-center justify-center transition-all duration-300 ${
                      isExpanded
                        ? "rotate-180 bg-red-600 text-white border-red-600"
                ...
                <div className="my-auto py-4">
                  ...
                  <p
                    className={`text-sm sm:text-lg font-medium mt-2 max-w-3xl transition-colors ${
                      isExpanded ? "text-red-900/80" : "text-gray-600"
                    }`}
                  >
                    {srv.shortDesc}
                  </p>
                ...
                <div
                  className={`flex items-center justify-between pt-4 border-t text-xs sm:text-sm font-bold transition-colors ${
                    isExpanded ? "border-red-100 text-red-600" : "border-gray-200 text-black"
                  }`}
                >
                  <div className="flex items-center gap-2">
                    <Icon className={`w-4 h-4 ${isExpanded ? "text-red-600" : "text-black"}`} />
              ...
              <AnimatePresence initial={false}>
                {isExpanded && (
                  <motion.div
                    ...
                    className="overflow-hidden border-t-2 border-red-600 bg-white p-6 sm:p-10 space-y-8 text-gray-900"
                  >
                    {/* WHAT WE PROVIDE TEXT LIST */}
                    <div className="space-y-4">
                      <div className="text-xs font-mono tracking-[0.25em] text-red-600 uppercase font-bold">
                        /// EVERYTHING WE PROVIDE IN THIS SERVICE
                      </div>

                      <div className="space-y-3">
                        {srv.providesList.map((item, pIdx) => (
                          <div key={pIdx} className="flex items-start gap-3 text-sm sm:text-base lg:text-lg font-bold text-gray-900">
                    ...
                    <div className="space-y-3 pt-4 border-t border-red-100">
                      <div className="text-xs font-mono tracking-widest text-red-600 uppercase font-bold">
                        KEY GUARANTEES & ADVANTAGES
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs sm:text-sm text-gray-900 font-medium">
                        {srv.featuresList.map((feat, fIdx) => (
                          <div key={fIdx} className="flex items-center gap-2.5 bg-red-50/50 p-3 rounded-xl border border-red-200">
                            <CheckCircle2 className="w-4 h-4 text-red-600 shrink-0" />
                            <span className="font-semibold text-gray-900">{feat}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="space-y-4 pt-4 border-t border-red-100">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {srv.popularOfferings.map((offering, oIdx) => (
                          <div
                            key={oIdx}
                            className="bg-red-50/40 border border-red-200 p-5 rounded-2xl space-y-3 flex flex-col justify-between"
                          >
                            <div className="space-y-1.5">
                              <div className="flex items-center justify-between text-xs text-red-700 font-mono">
                                <span>{offering.duration}</span>
                                <span className="font-extrabold text-red-600 text-sm">{offering.price}</span>
                              </div>
                              <h4 className="text-base font-extrabold text-gray-900">{offering.name}</h4>
                            ...
                            <button
                              ...
                              className="w-full py-2.5 bg-red-600 hover:bg-red-700 text-white text-xs font-bold rounded-xl transition flex items-center justify-center gap-2 shadow-sm"
                            >
                    ...
                    {/* BOTTOM ACTION */}
                    <div className="pt-4 flex items-center justify-between border-t border-red-100">
                      <span className="text-xs font-semibold text-gray-600">24/7 Dedicated Support Desk Available</span>
                      <button
                        onClick={() => setModalTitle(srv.title)}
                        className="px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-extrabold text-xs rounded-xl shadow-md transition flex items-center gap-2"
                      >
        ...
        {modalTitle && (
          <div className="fixed inset-0 z-[999] bg-black/70 backdrop-blur-md flex items-center justify-center p-4">
            <motion.div
              ...
              className="bg-white text-black border-2 border-red-600 rounded-3xl max-w-lg w-full p-6 space-y-6 shadow-2xl relative"
            >
              <div className="flex items-center justify-between border-b border-gray-200 pb-4">
                <div>
                  <div className="text-[10px] font-mono font-bold text-red-600 uppercase">QUICK INQUIRY</div>
              ...
              >
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-gray-50 border border-gray-300 rounded-xl px-4 py-2.5 text-xs text-black outline-none focus:border-red-600"
                  />
                </div>
                ...
                <button
                  type="submit"
                  className="w-full py-3.5 bg-red-600 hover:bg-red-700 text-white font-black text-xs rounded-xl transition shadow-lg flex items-center justify-center gap-2"
                >
